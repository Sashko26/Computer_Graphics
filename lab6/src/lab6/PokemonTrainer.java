...
public class PokemonTrainer extends JFrame{
    ...
    public void createSceneGraph(SimpleUniverse su){
        ...
        TextureLoader t = new TextureLoader("source_folder//bf.jpg", myCanvas3D);
        ...
        // голова
		 ...
		 Appearance headApp = new Appearance(); setToMyDefaultAppearance(headApp, new
		 Color3f(0.2f, 0.2f, 0.5f));
		 
		 Alpha headRotAlpha = new Alpha(movesCount, Alpha.INCREASING_ENABLE,
		 startTime, 0, movesDuration,0,0,0,0,0);
		 
		 Shape3D head = (Shape3D) roachNamedObjects.get("polygon1");
		 head.setAppearance(headApp); TransformGroup headTG = new TransformGroup();
		 headTG.addChild(head.cloneTree());
		 
		  Transform3D headRotAxis = new Transform3D(); headRotAxis.set(new
		 Vector3d(0.0, 0.0, 0.0));
		  
		  RotationInterpolator headRot = new RotationInterpolator(headRotAlpha, headTG,
		  headRotAxis, 0.0f, (float) Math.PI*2); headRot.setSchedulingBounds(bs);
